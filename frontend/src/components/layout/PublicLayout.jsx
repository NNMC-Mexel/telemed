...
import { Menu, X, Activity, Phone, Mail, MapPin, ChevronDown, UserCircle, Stethoscope } from 'lucide-react'
...
function PublicLayout() {
  ...
  const navLinks = [
    ...
    { href: '#specializations', label: 'Специализации', isAnchor: location.pathname === '/' },
    { href: '#about', label: 'О нас', isAnchor: location.pathname === '/' },
    { href: '#contact', label: 'Контакты', isAnchor: location.pathname === '/' },
  ]
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={cn(
                    ...
                    showDarkHeader ? 'text-white/90' : 'text-slate-700',
                    location.pathname === link.href && !link.isAnchor && 'text-teal-500'
                  )}
                >
                  {link.label}
                </Link>
              ...
              {isAuthenticated ? (
                ...
                <>
                  ...
                  <div className="relative" ref={loginDropdownRef}>
                    <Button 
                      variant={showDarkHeader ? 'outline' : 'ghost'} 
                      className={showDarkHeader ? 'text-white border-white/30 hover:bg-white/10' : ''}
                      onClick={() => setShowLoginDropdown(!showLoginDropdown)}
                      rightIcon={<ChevronDown className={cn('w-4 h-4 transition-transform', showLoginDropdown && 'rotate-180')} />}
                    >
                      Войти
                    </Button>
                    ...
                    {showLoginDropdown && (
                      <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-slideDown">
                        ...
                        >
                          ...
                          <div>
                            <p className="font-medium text-slate-900">Пациент</p>
                            <p className="text-xs text-slate-500">Личный кабинет пациента</p>
                          </div>
                        </Link>
                        <Link
          ...
          <nav className="flex flex-col p-4 gap-2">
            ...
            <div className="pt-4 mt-2 border-t border-slate-100 flex flex-col gap-2">
              {isAuthenticated ? (
                ...
                <>
                  <p className="text-xs text-slate-500 mb-2 px-1">Вход в личный кабинет</p>
                  <Link to="/login?type=patient">
                    <Button variant="secondary" className="w-full justify-start" leftIcon={<UserCircle className="w-4 h-4" />}>
                      Войти как пациент
                    </Button>
                  </Link>
            ...
            <div>
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Телемедицина</p>
                </div>
              </div>
              <p className="text-slate-400 text-sm leading-relaxed">
                Современная платформа телемедицины для онлайн-консультаций с квалифицированными врачами.
              </p>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-4">
                ...
                <li className="flex items-start gap-3 text-slate-400">
                  ...
                  <span className="text-sm">г. Астана, пр. Мангилик Ел, 55</span>
                </li>
              </ul>
            </div>
          </div>
